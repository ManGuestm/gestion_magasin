...
class _VentesModalState extends State<VentesModal> with TabNavigationMixin {
  ...
  List<CltData> _filterClientsByRole(List<CltData> allClients) {
    if (widget.tousDepots) {
      // Tous dépôts: seuls les clients avec catégorie "Tous dépôts"
      return allClients.where((client) => client.categorie == ClientCategory.tousDepots.label).toList();
    } else {
      // MAG: seuls les clients avec catégorie "Magasin"
      return allClients.where((client) => client.categorie == ClientCategory.magasin.label).toList();
    }
  }
  ...
  void _validerQuantite(String value) async {
    ...
    if (quantite > _stockDisponible && !_stockWarningShown) {
      ...
      // Après confirmation "Continuer", mettre le focus sur le champ P.U HT
      Future.delayed(const Duration(milliseconds: 100), () {
        if (mounted) {
                            ...
                            child: pw.Column(
                              ...
                              children: [
                                ...
                                if (_selectedFormat != 'A6') ...[
                                  ...
                                  if (societe?.rcs != null)
                                    pw.Text(
                                      "RCS: ${societe!.rcs!} ${societe.cif != null ? " | CIF: ${societe.cif!}" : ""}",
                                      style: pw.TextStyle(fontSize: pdfFontSize - 1),
                                    ),
                                  if (societe?.nif != null)
                                    pw.Text(
                                      "NIF: ${societe!.nif!} ${societe.stat != null ? " | STAT: ${societe.stat!}" : ""}",
                                      style: pw.TextStyle(fontSize: pdfFontSize - 1),
                                    ),
                                  if (societe?.port != null)
                                    pw.Text(
                                      "Tél: ${societe!.port!} ${societe.email != null ? " | Email: ${societe.email!}" : ""}",
                                      style: pw.TextStyle(fontSize: pdfFontSize - 1),
                                    ),
                                ],

                                if (societe?.port != null && _selectedFormat == 'A6')
                                  pw.RichText(
                                    text: pw.TextSpan(
                                      text: 'Tél: ${societe!.port!}',
                                      style: pw.TextStyle(fontSize: pdfFontSize - 1, color: PdfColors.black),
                                      children: [
                                        pw.TextSpan(
                                          text: ' | RCS: ${societe.rcs!}',
                                          style: pw.TextStyle(
                                ...
                                if (_selectedFormat == 'A6')
                                  _buildPdfInfoRow('Doit:', _selectedClient!, pdfFontSize),
                              ],
                            ),
                          ),
                          pw.Expanded(
                            ...
                            child: pw.Column(
                              crossAxisAlignment: pw.CrossAxisAlignment.start,
                              children: [
                                _buildPdfInfoRow('FACTURE N°:', _nFactureController.text, pdfFontSize),
                                _buildPdfInfoRow('DATE:', _dateController.text, pdfFontSize),
                                if (_selectedFormat != 'A6')
                                  _buildPdfInfoRow('Doit:', _selectedClient!, pdfFontSize),
                                if (_selectedFormat != 'A6')
                                  _buildPdfInfoRow('Adresse:', _selectedClient!, pdfFontSize),
                                _buildPdfInfoRow('Mode de paiement:', _selectedModePaiement!, pdfFontSize),
                              ],
                            ),
                          ),
                        ],
                        ...
                        children: [
                          ...
                          pw.Expanded(
                            ...
                            child: pw.Column(
                              ...
                              children: [
                                _buildPdfInfoRow('DATE:', _dateController.text, pdfFontSize),
                                _buildPdfInfoRow('BON DE LIVRAISON:', _nFactureController.text, pdfFontSize),
                                _buildPdfInfoRow('DOIT:', _selectedClient!, pdfFontSize),
                              ],
                            ),
                          ),
  ...
  pw.Widget _buildPdfTableCell(
    ...
    return pw.Container(
      alignment: isArticle
          ...
          : pw.Alignment.center,
      padding: pw.EdgeInsets.all(_selectedFormat == 'A6' ? 8.0 : (_selectedFormat == 'A5' ? 3.0 : 5.0)),
      child: pw.Text(
        text,
        style: pw.TextStyle(
          fontSize: fontSize - 1,
