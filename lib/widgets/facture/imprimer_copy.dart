...
class PdfGenerator {
          ...
          return pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                // En-tête avec infos société et document
                _buildHeader(),
    ...
    return pw.Column(
      ...
      children: [
        ...
        if (config.selectedFormat != 'A6') ...[
          ...
          _buildInfoRow('Adresse:', config.client),
        ],
  ...
  pw.Widget _buildArticlesTable() {
    return pw.Container(
      padding: pw.EdgeInsets.all(config.padding / 2),
      child: pw.Column(children: [_buildTableHeader(), _buildTableData()]),
    );
  }
  ...
  /// Construit les données du tableau
  pw.Widget _buildTableData() {
    final columnWidths = config.showDepot && config.selectedFormat != 'A6'
        ? {
            0: const pw.FlexColumnWidth(4),
            1: const pw.FlexColumnWidth(1),
    ...
    return pw.Table(
      ...
      columnWidths: columnWidths,
      children: config.lignes.map((ligne) => _buildTableRow(ligne)).toList(),
    );
  }
  ...
  static Future<void> printDocument(
    ...
    try {
      ...
      if (context.mounted) {
        _showSnackBar(context, '${config.documentTitle} envoyée à l\'impression');
      }
    } catch (e) {
      if (context.mounted) {
        _showSnackBar(context, 'Erreur d\'impression: $e', isError: true);
      }
    }
  }
