...
class NetworkDatabaseService {
  ...
  Future<Article?> getArticleByDesignation(String designation) async {
    final result = await customSelect('SELECT * FROM articles WHERE designation = ?', [Variable(designation)]);
  ...
  Future<bool> userExists(String username) async {
    final result = await customSelect('SELECT COUNT(*) as count FROM users WHERE nom = ?', [Variable(username)]);
    return (result.first['count'] as int) > 0;
  }

  Future<User?> getUserByCredentials(String username, String password) async {
    final result = await customSelect('SELECT * FROM users WHERE username = ? AND motDePasse = ?', [Variable(username), Variable(password)]);
    if (result.isEmpty) return null;
  ...
  Future<double> getTotalVentes() async {
    final result = await customSelect('SELECT COALESCE(SUM(totalttc), 0) as total FROM ventes WHERE contre != "1"');
    return (result.first['total'] as num).toDouble();
  }

  Future<double> getVentesToday() async {
    final today = DateTime.now().toIso8601String().split('T')[0];
    final result = await customSelect('SELECT COALESCE(SUM(totalttc), 0) as total FROM ventes WHERE DATE(daty) = ? AND contre != "1"', [Variable(today)]);
    return (result.first['total'] as num).toDouble();
  }
}